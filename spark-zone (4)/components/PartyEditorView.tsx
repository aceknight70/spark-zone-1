...
const StageSetupSection: React.FC<{ partyData: Party; setPartyData: React.Dispatch<React.SetStateAction<Party>> }> = ({ partyData, setPartyData }) => {
    ...
    const addToken = (charId: number) => {
        if (!partyData.stage.tabletop?.pinnedCharacterIds.includes(charId)) {
            setPartyData(prev => ({
                ...prev,
                stage: {
                    ...prev.stage,
                    tabletop: {
                        ...prev.stage.tabletop,
                        mapUrl: prev.stage.tabletop?.mapUrl || null,
                        pinnedCharacterIds: [...(prev.stage.tabletop?.pinnedCharacterIds || []), charId]
                    }
                }
            }));
        }
        ...
        setPartyData(prev => ({
            ...
            stage: {
                ...
                tabletop: {
                    ...prev.stage.tabletop,
                    mapUrl: prev.stage.tabletop?.mapUrl || null,
                    pinnedCharacterIds: prev.stage.tabletop?.pinnedCharacterIds.filter(id => id !== charId) || []
                }
            }
        }));
    };
    ...
    const pinnedChars = (partyData.stage.tabletop?.pinnedCharacterIds || [])
                ...
                <div className="bg-gray-800/40 p-6 rounded-xl border border-violet-500/20 animate-fadeIn space-y-6">
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <label className="block text-sm font-medium text-gray-300">Manage Tokens</label>
                        ...
                        <div className="space-y-2">
                            {pinnedChars.map(char => (
                                <div key={char!.id} className="flex items-center justify-between bg-black/30 p-2 rounded-lg">
                            ...
                            {pinnedChars.length === 0 && <p className="text-xs text-gray-500 italic">No tokens added yet.</p>}
                        </div>
                    </div>
                </div>
            )}
